# NextLite Voice V3 — Complete Agent Lifecycle Audit

> **Audit Type**: End-to-End Agent State Machine & Deployment Lifecycle  
...
> **"Which exact database row/configuration does the voice worker use for a production call?"**
...
4. The worker uses the **exact `agent_versions` row whose primary key `id` equals `deployments.version_id`**, where `deployments.id` is the `deploymentId` provided in the incoming telephony/SIP/WebSocket session metadata, AND `deployments.environment = 'PRODUCTION'`, AND `deployments.status = 'ACTIVE'`.
