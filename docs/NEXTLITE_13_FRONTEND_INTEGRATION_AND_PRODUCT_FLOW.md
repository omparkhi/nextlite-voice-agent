# NextLite Voice V3 — Frontend Integration & Product Flow Architecture
## Document 13: Admin Studio, Client CRM & UI-to-Runtime Traceability

> **Document Type**: Frontend Architecture & UI-to-Runtime Mapping  
...
1. `config.runtimeSettings.noiseCancellationModel`: Supported in `RuntimeAgentConfig.runtime.noiseCancellationModel` and backend schema, but `SettingsEditor.tsx` does not render a selector for noise cancellation models.
...
2. `agent_tools` table: Excluded from runtime resolution; UI edits `agent_versions.configuration.tools.bindings`.
