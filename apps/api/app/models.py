...
class Subscription(Base):
    ...
    minutesLimit: Mapped[int] = mapped_column("minutes_limit", Integer, default=60, nullable=False)
    minutesUsed: Mapped[int] = mapped_column("minutes_used", Integer, default=0, nullable=False)
    currentPeriodStart: Mapped[datetime] = mapped_column("current_period_start", DateTime, default=datetime.utcnow, nullable=False)
...
class FollowUp(Base):
    ...
    tenantId: Mapped[uuid.UUID] = mapped_column("tenant_id", UUID(as_uuid=True), ForeignKey("tenants.id", ondelete="CASCADE"), nullable=False)
    callSessionId: Mapped[Optional[uuid.UUID]] = mapped_column("call_session_id", UUID(as_uuid=True), ForeignKey("call_sessions.id", ondelete="SET NULL"), nullable=True)
    recipientPhone: Mapped[str] = mapped_column("recipient_phone", String(50), nullable=False)
    recipientName: Mapped[Optional[str]] = mapped_column("recipient_name", String(255), nullable=True)
    channel: Mapped[str] = mapped_column(String(50), default="whatsapp", nullable=False)
    templateName: Mapped[Optional[str]] = mapped_column("template_name", String(100), nullable=True)
    messageContent: Mapped[Optional[str]] = mapped_column("message_content", Text, nullable=True)
    status: Mapped[FollowUpStatus] = mapped_column(SQLEnum(FollowUpStatus, name="follow_up_status", native_enum=False), nullable=False, default=FollowUpStatus.PENDING)
    sentAt: Mapped[Optional[datetime]] = mapped_column("sent_at", DateTime, nullable=True)
